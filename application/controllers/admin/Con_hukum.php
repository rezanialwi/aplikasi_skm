...
redirect(base_url().'login?alert=belum_login');
	...
	function index(){
      ...
      $config['page_query_string'] = TRUE;
  		$config['base_url'] 				 = base_url().'admin/con_hukum/?cari='.$cari;
  		$config['total_rows'] 			 = $this->mod_hukum->jumlah_row($search);
      ...
      $data['all_hukum'] = $this->mod_hukum->get($batas,$offset,$search);

		$this->load->view('lte/v_header');
		$this->load->view('lte/v_hukum', $data);
	...
	$data['responden'] = $this->mod_hukum->edit_data($where,'responden')->result();
	$this->load->view('lte/v_header');
		$this->load->view('lte/v_hukum_edit', $data);
		$this->load->view('lte/v_footer');
    }
	...
	$this->mod_hukum->update_data($where,$data,'responden');
	redirect('admin/con_hukum/index');
}

    function hapus($id_responden){
		$where = array('id_responden' => $id_responden);
		$this->mod_hukum->hapus_data($where,'responden');
		redirect('admin/con_hukum/index');
	}

	 function search(){
		$keyword = $this->input->post('keyword');
		$data['all_hukum']=$this->mod_hukum->get_product_keyword($keyword);
		$this->load->view('lte/v_header');
		$this->load->view('lte/v_hukum_cari',$data);
		$this->load->view('lte/v_footer');
  }
}