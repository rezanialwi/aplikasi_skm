...
redirect(base_url().'login?alert=belum_login');
	...
	function index(){
      ...
      $config['page_query_string'] = TRUE;
  		$config['base_url'] 				 = base_url().'admin/con_phi/?cari='.$cari;
  		$config['total_rows'] 			 = $this->mod_phi->jumlah_row($search);
      ...
      $data['all_phi'] = $this->mod_phi->get($batas,$offset,$search);

		$this->load->view('lte/v_header');
		$this->load->view('lte/v_phi', $data);
	...
	$data['responden'] = $this->mod_phi->edit_data($where,'responden')->result();

	
	$this->load->view('lte/v_header');
	$this->load->view('lte/v_phi_edit',$data);
		$this->load->view('lte/v_footer');
    }
	...
	$this->mod_phi->update_data($where,$data,'responden');
	redirect('admin/con_phi/index');
}

    function hapus($id_responden){
		$where = array('id_responden' => $id_responden);
		$this->mod_phi->hapus_data($where,'responden');
		redirect('admin/con_phi/index');
	}
	 ...
	 function search(){
		...
		$data['all_phi']=$this->mod_phi->get_product_keyword($keyword);
		$this->load->view('lte/v_header');
		$this->load->view('lte/v_phi_cari',$data);
		$this->load->view('lte/v_footer');
  }
}