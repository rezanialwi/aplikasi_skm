...
redirect(base_url().'login?alert=belum_login');
	...
	function index(){
	 $cari = $this->input->get('cari');
      ...
      $config['page_query_string'] = TRUE;
  		$config['base_url'] 				 = base_url().'admin/con_perdata/?cari='.$cari;
  		$config['total_rows'] 			 = $this->mod_perdata->jumlah_row($search);
      ...
      $data['all_perdata'] = $this->mod_perdata->get($batas,$offset,$search);

		$this->load->view('lte/v_header');
		$this->load->view('lte/v_perdata', $data);
	...
	$data['responden'] = $this->mod_perdata->edit_data($where,'responden')->result();
	$this->load->view('lte/v_header');
	$this->load->view('lte/v_perdata_edit',$data);
		$this->load->view('lte/v_footer');
	...
	$this->mod_perdata->update_data($where,$data,'responden');
	redirect('admin/con_perdata/index');
}

    function hapus($id_responden){
		$where = array('id_responden' => $id_responden);
		$this->mod_perdata->hapus_data($where,'responden');
		redirect('admin/con_perdata/index');
	}

	 function search(){
		$keyword = $this->input->post('keyword');
		$data['all_perdata']=$this->mod_perdata->get_product_keyword($keyword);
		$this->load->view('lte/v_header');
		$this->load->view('lte/v_perdata_cari',$data);
		$this->load->view('lte/v_footer');
  }
}