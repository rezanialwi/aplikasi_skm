...
redirect(base_url().'login?alert=belum_login');
	...
	function index(){
      ...
      $config['page_query_string'] = TRUE;
  		$config['base_url'] 				 = base_url().'admin/con_umum/?cari='.$cari;
  		$config['total_rows'] 			 = $this->mod_umum->jumlah_row($search);
      ...
      $data['all_umum'] = $this->mod_umum->get($batas,$offset,$search);

		$this->load->view('lte/v_header');
		$this->load->view('lte/v_umum', $data);
	...
	$data['responden'] = $this->mod_umum->edit_data($where,'responden')->result();

	$this->load->view('lte/v_header');
	$this->load->view('lte/v_umum_edit',$data);
	$this->load->view('lte/v_footer');
    }
	...
	$this->mod_umum->update_data($where,$data,'responden');
	redirect('admin/con_umum/index');
}

    function hapus($id_responden){
		$where = array('id_responden' => $id_responden);
		$this->mod_umum->hapus_data($where,'responden');
		redirect('admin/con_umum/index');
	}

	function search(){
		$keyword = $this->input->post('keyword');
		$data['all_umum']=$this->mod_umum->get_product_keyword($keyword);
		$this->load->view('lte/v_header');
		$this->load->view('lte/v_umum_cari',$data);
		$this->load->view('lte/v_footer');
  }
}